...
export interface GraphQLMe {
  ...
  work: GraphQLWork[];
  skills: string[];
  achievements?: GraphQLProject[];
  workProjects: GraphQLProject[];
...
export function resumeDataToGraphQL(data: ResumeData): GraphQLMe {
  // Handle avatarUrl conversion from StaticImageData to string if needed
  let avatarUrl: string;
  if (typeof data.avatarUrl === "string") {
    avatarUrl = data.avatarUrl;
  } else {
    // For StaticImageData, we'll use the src property if available
    // or convert to empty string as fallback
    avatarUrl = data.avatarUrl.src || "";
  }
  ...
  return {
    ...
    achievements: data.achievements?.map((achievement) => ({
      ...
      techStack: [],
      description: reactToString(achievement.description),
      link: achievement.issuer
        ? { label: achievement.issuer, href: "" }
        : undefined,
    })),
    workProjects: data.workProjects.map((project) => ({
      title: project.title,
